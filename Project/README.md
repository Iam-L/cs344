...
borg-SLO-classifiers: contains datasets and other material relating to the SLO-classifiers repository stored on Calvin's Borg supercomputer (local system only; not included in this repository)

deprecated-materials: project materials no longer in use.

preprocessed-datasets: datasets pre-processed using slo_tbl_preprocessor.py.

project-written-documents: text documentation discussing the project.

summer-research-pdfs: source material referenced by this project.

tbl-datasets: small datasets provided by Professor VanderLinden that we use to preprocess and train our models on.

&nbsp;

## Jupyter Notebook Files:

proposal(final).ipynb: Deprecated.  Can ignore.

report.ipynb: Contains project discussions including vision statements, background information, code implementation, research results, and research implications.
...
Run slo_tbl_preprocessor.py

### Description:

The Tweet preprocessor contains 3 separate functions that each pre-processes a specific dataset.  Comment/Uncomment the associated functions in the driver program "main" to pre-process the datasets.
...
Note: The large dataset is NOT included in the GitHub repository and must be obtained separately (over 300 MBs CSV file size).  The two smaller datasets ARE included in the GitHub repository and do not need to be obtained.
...
Run slo_topic_classification_v2-0.py (version 1 is deprecated)

### Description:

The Tweet topic classification program is currently one large monolithic codebase that sequentially trains, predicts, and visualizes the results of various Scikit-Classifiers.  None of these Classifiers are neural networks except the Multi-Layer Perceptron Classifier.

Note: The required datasets assumes a relative path from the root project directory of:

- /preprocessed-datasets/

&nbsp;

The Python code will automatically import the datasets and prep them to be used as a input feature and target label set.  Then, it will run various Classifiers and output metric summarizations and visualizations for each one to the console output.  You can control which Classifier is run by commenting/uncommenting the relevant function calls in the "main" driver program at the very bottom of the codebase!
...
EndNote: For specific details concerning our code implementation refer to the Python files.  The code is well-documented.  Otherwise, refer to report.ipynb "implementation" section for a explanation of the implementation details.

&nbsp;

## GitHub Repositories:

SLO Research Repository:
https://github.com/Calvin-CS/slo-classifier

&nbsp;

Joseph Jinn - CS 344 Repository:
